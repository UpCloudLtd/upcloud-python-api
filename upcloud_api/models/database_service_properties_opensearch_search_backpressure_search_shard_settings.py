...
T = TypeVar("T", bound="DatabaseServicePropertiesOpensearchSearchBackpressureSearchShardSettings")


@_attrs_define
class DatabaseServicePropertiesOpensearchSearchBackpressureSearchShardSettings:
    """
    Attributes:
        cancellation_burst (float | Unset): The maximum number of search tasks to cancel in a single iteration of the
            observer thread. Default is 10.0
        cancellation_rate (float | Unset): The maximum number of tasks to cancel per millisecond of elapsed time.
            Default is 0.003
        cancellation_ratio (float | Unset): The maximum number of tasks to cancel, as a percentage of successful task
            completions. Default is 0.1
        cpu_time_millis_threshold (int | Unset): The CPU usage threshold (in milliseconds) required for a single search
            shard task before it is considered for cancellation. Default is 15000
        elapsed_time_millis_threshold (int | Unset): The elapsed time threshold (in milliseconds) required for a single
            search shard task before it is considered for cancellation. Default is 30000
        heap_moving_average_window_size (int | Unset): The number of previously completed search shard tasks to consider
            when calculating the rolling average of heap usage. Default is 100
        heap_percent_threshold (float | Unset): The heap usage threshold (as a percentage) required for a single search
            shard task before it is considered for cancellation. Default is 0.5
        heap_variance (float | Unset): The minimum variance required for a single search shard task’s heap usage
            compared to the rolling average of previously completed tasks before it is considered for cancellation. Default
            is 2.0
        total_heap_percent_threshold (float | Unset): The heap usage threshold (as a percentage) required for the sum of
            heap usages of all search shard tasks before cancellation is applied. Default is 0.5
    """
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        database_service_properties_opensearch_search_backpressure_search_shard_settings = cls(
        ...
        return database_service_properties_opensearch_search_backpressure_search_shard_settings
