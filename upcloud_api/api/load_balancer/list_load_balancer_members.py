...
) -> LoadBalancerErrorResponse | list[LoadBalancerMember]:
    if response.status_code == 200:
        response_200 = []
        _response_200 = response.json()
        for componentsschemasload_balancer_members_response_item_data in _response_200:
            componentsschemasload_balancer_members_response_item = LoadBalancerMember.from_dict(
                componentsschemasload_balancer_members_response_item_data
            )

            response_200.append(componentsschemasload_balancer_members_response_item)
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[LoadBalancerErrorResponse | list[LoadBalancerMember]]:
    return Response(
...
def sync(
    ...
    *,
    client: AuthenticatedClient | Client,
) -> LoadBalancerErrorResponse | list[LoadBalancerMember] | None:
    """List load balancer members
